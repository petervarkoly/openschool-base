...
sub check_pw($)
{
	...
	if( $check )
	{
                my $E = cmd_pipe('/usr/sbin/cracklib-check',$pw); chomp($E);
                $E =~ s/^.*: OK$//;
                $error .= $E;
	}
        return $error;
...
sub make_array_attributes($)
{
        ...
        foreach(keys %$hash )
        {
                if(/^member|memberOff|memberUid|newmember|group|newgroup|rasaccess|class$/i || 
		   /mailAcceptAddress|mailForwardAddress|zarafasendasprivilege$/i)
                {
                        my @a = split /\n/, $hash->{$_};
			if( scalar @a )
			{
